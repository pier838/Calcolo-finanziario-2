...
def simulate(p, housing, down_pct=0.0, lifecycle="keep"):
    ...
    for age in range(age0, age_end+1):
        y = age-age0
        gross = p["gross_salary"]*((1+p["salary_growth"])**y) if age < retire_age else 0.0
...
with st.sidebar:
    ...
    st.header("Reddito e imposte")
    gross_salary=st.number_input("RAL iniziale (€)",0.0,value=float(gv("gross_salary",68000.0)),step=1000.0)
    salary_growth=st.number_input("Crescita RAL annua (%)",0.0,10.0,float(gv("salary_growth",0.02))*100,0.1)/100
...
p=dict(
    ...
    gross_salary=gross_salary,salary_growth=salary_growth,worker_contrib_rate=worker_contrib_rate,
